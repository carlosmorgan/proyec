...
use App\Http\Requests\IngresoUserFormRequest;
use App\IngresoUser;
use App\DetalleIngresoUser;
use DB;
use Carbon\Carbon;
use Response;
use Illuminate\Support\Collection;
class ComprasUserController extends Controller
{
 ...
 public function __construct()
    ...
    public function index(Request $request)
    {
        if ($request)
        {
            ...
            ->paginate(7);
            return view('user.almacen.compras.index',["ingresos"=>$ingresos,"searchText"=>$query]);
        }
...
public function create()
{
	...
	->get();
return view("user.compras.create",["personas"=>$personas, "articulos"=>$articulos]);
}


 public function store (IngresoUserFormRequest $request)
    {
      ...
      $cantidad=$request->get('cantidad');
        $precio_compra=$request->get('precio_compra');
          ...
          $cont = 0;
           while($cont < count($idarticulo)){
             $detalle = new DetalleIngresoUser();
        ...
        return Redirect::to('user/compras');
     ...
     public function show($id)
    {
     $ingreso=DB::table('compras as i')
        ...
        $detalles=DB::table('detalle_compras as d')
             ...
             ->get();
        return view("user.compras.show",["ingreso"=>$ingreso,"detalles"=>$detalles]);
    }


public function destroy($id)
    {
     $ingreso=IngresoUser::findOrFail($id);
        $ingreso->Estado='C';
        $ingreso->update();
        return Redirect::to('user/compras');
    }
